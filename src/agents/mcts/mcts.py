...
class UCTAgent(object):
    ...
    def best_child(self, parent_node, parent_state):
        # use UCT/UCB1 to select child node
        N = parent_node.N
        max_uct_node = None
    ...
    def tree_policy(self, parent_node, parent_state):
        ...
        # selection, finding the most urgent expandable node
        while not self.env.check_terminal(parent_state):
            if not parent_node.is_full_expanded():
                ...
                # select child node based on UCB1/UCT
    ...
    def default_policy(self, state):
        ...
        # simulation, random play turn-by-turn
        # until simulated game is done
        steps = 0
    ...
    def backup_negamax(self, node, outcome):
        ...
        # backpropagate outcome to root node
        while node is not None:
            ...
            outcome = -outcome # two-player, zero-sum, adversarial games
            node = node.parent
    ...
    def final_action(self, root_node, initial_state):
        ...
        for a in sorted(root_node.children):
            ...
            # select best action based on node with greatest number of visits
